...
class Genome {
  constructor(inp, out, id, offSpring = false) {
    ...
    this.layers = 2; // Number of layers for the neural network
    this.nextNode = 0;
    ...
    if (!offSpring) {
      //This is not an offspring genome generate a fullyConnected net
  ...
  crossover(partner) {
    ...
    for (let i = 0; i < this.connections.length; i++) {
      let index = this.commonConnection(
        this.connections[i].getInnovationNumber(),
        partner.connections
      );

      if (index != -1) {
        //There is a commonConnection
        let conn =
          Math.random() > 0.5
  ...
  addConnection() {
    ...
    if (this.fullyConnected()) return; //Cannot add connections if it's fullyConnected
  ...
  commonConnection(innN, connections) {
    ...
    for (let i = 0; i < connections.length; i++) {
      if (innN == connections[i].getInnovationNumber()) return i;
    }
  ...
  fullyConnected() {
    ...
    this.nodes.forEach((node) => {
      if (nodesPerLayer[node.layer] != undefined) nodesPerLayer[node.layer]++;
      else nodesPerLayer[node.layer] = 1;
    });
  ...
  getNode(x) {
    ...
    for (let i = 0; i < this.nodes.length; i++)
      if (this.nodes[i].number == x) return i;
